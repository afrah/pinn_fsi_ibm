...
class Trainer(BaseTrainer):
    ...
    def _run_epoch(self, epoch):
        ...
        self.scheduler_force.step(total_loss_force)
    ...
    def _compute_losses_force(self):
        ...
        batch_indices = self.get_random_minibatch(
            self.train_dataloader.fluid_data.txy_fluid_points.shape[0], 256
        )
        ...
        batch_indices = self.get_random_minibatch(
            self.train_dataloader.solid_data.txy_solid_points.shape[0], 256
        )
        ...
        batch_indices = self.get_random_minibatch(
            self.train_dataloader.fluid_data.txy_initial.shape[0], 256
        )
        txy_intitial = self.train_dataloader.fluid_data.txy_initial[batch_indices, :]
        uvp_intitial = self.train_dataloader.fluid_data.uvp_initial[batch_indices, :]

        pred_initial = self.model_force(
            txy_intitial,
            self.train_dataloader.fluid_data.min_x[:3],
            self.train_dataloader.fluid_data.max_x[:3],
        )
        linitial = torch.mean(
        ...
        lfluid = torch.mean(
            # torch.square(pred_fluid[:, 0] - uvp_solid[:, 0])
            # + torch.square(pred_fluid[:, 1] - uvp_solid[:, 1])
            # + torch.square(pred_fluid[:, 2] - uvp_solid[:, 2])
            torch.square(pred_fluid[:, 0] - uvp_fluid[:, 3])
            + torch.square(pred_fluid[:, 1] - uvp_fluid[:, 4])
        )

        return {"force_points": 2.0 * (lfluid + linitial)}

    def get_mini_batch_data(self, data):
        data_mean = data.min_x[:3]
        data_std = data.max_x[:3]
