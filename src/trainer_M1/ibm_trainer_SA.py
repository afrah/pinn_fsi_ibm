            ...
            self.weights = {
                key: nn.Parameter(
                    torch.rand(
                        ...
                        device=self.rank,
                        requires_grad=True,
                    )  # Place dtype, device, and requires_grad here
                )
        ...
        if self.config.get("weighting") == "SA":
            ...
            if epoch % 1000 == 0:
                for key, value in self.weights.items():
                    print(f"Mean of {key} weights : {torch.mean(value).item():.2e}")
            total_loss = sum(
                [
                    torch.mean(
                        torch.square(
                            self.weights.get("left") * torch.sqrt(bclosses["left"])
                        )
                    ),
                    torch.mean(
                        torch.square(
                            self.weights.get("right") * torch.sqrt(bclosses["right"])
                        )
                    ),
                    torch.mean(
                        torch.square(
                            self.weights.get("bottom") * torch.sqrt(bclosses["bottom"])
                        )
                    ),
                    torch.mean((self.weights.get("up") * torch.sqrt(bclosses["up"]))),
                    torch.mean(
                        torch.square(
                            self.weights.get("fluid_points")
                            * torch.sqrt(bclosses["fluid_points"])
                        )
                    ),
                    torch.mean(
                        torch.square(
                            self.weights.get("initial")
                            * torch.sqrt(bclosses["initial"])
                        )
                    ),
                    torch.mean(
                        torch.square(
                            self.weights.get("fluid") * torch.sqrt(bclosses["fluid"])
                        )
                    ),
                ]
            )
