...
class PINNTrainer:
    ...
    def __init__(
        ...
        Args:
            model: PINN model
            training_data: Dictionary containing different parts of the domain
                with their respective data
            logger: Logger object
            fluid_density: Density of the fluid
            fluid_viscosity: Dynamic viscosity of the fluid
            print_every: Number of epochs between print statements
            ...
            for epoch in range(num_epochs):
                ...
                for domain_type, loader in data_loaders.items():
                    if domain_type in self.loss_list:
                        ...
                        for batch_idx, (batch_tensor,) in enumerate(loader):
                            ...
                            loss = torch.tensor(0.0, requires_grad=True).to(self.device)
                            solid_loss = torch.tensor(0.0, requires_grad=True).to(
                                self.device
                            )
                            ...
                            if domain_type in [
                                ...
                                loss = interface_loss  # solid_loss +  #+ fsi_loss
                ...
                if (epoch + 1) % self.print_every == 0:
                    ...
                    self.logger.print(
                        ...
                        f"Data(F&S): {sum(epoch_losses[b].item() for b in ['solid', 'fluid_points']):.1e}, "
